...
void MultiMeshInstance3D::OnTreeEnter() {
	Node3D::OnTreeEnter();
	if (m_Tree != nullptr) {
		m_Tree->GetBatchRenderer()->RegisterMultiMesh(this);
	}
}
...
void MultiMeshInstance3D::OnTreeExit() {
	Node3D::OnTreeExit();
	if (m_Tree != nullptr) {
		m_Tree->GetBatchRenderer()->UnregisterMultiMesh(this);
	}
}

void MultiMeshInstance3D::SetMeshAsset(std::shared_ptr<Asset::Mesh> meshAsset) noexcept {
	m_MeshAsset = std::move(meshAsset);
}

std::shared_ptr<Asset::Mesh> MultiMeshInstance3D::GetMeshAsset() const noexcept {
	return m_MeshAsset;
}

void MultiMeshInstance3D::SetMaterial(std::shared_ptr<Render::Material> material) noexcept {
	m_Material = std::move(material);
}

std::shared_ptr<Render::Material> MultiMeshInstance3D::GetMaterial() const noexcept {
	return m_Material;
}

Render::Mesh * MultiMeshInstance3D::GetMesh() const noexcept {
...
void MultiMeshInstance3D::SetMesh(std::shared_ptr<Render::Mesh> proceduralMesh) noexcept {
	...
	m_MeshAsset->FilePath = "Procedural_Multi_" + std::to_string(m_ID);
