...
void main() {
    ...
    float roughness = max(texture(u_RoughnessMap, v_TextureCoordinate).r * u_RoughnessFactor, 0.045);
    ...
    for (int a = 0; a < u_PointLightCount; ++a) {
        ...
        float rangeSquared = max(light.Range * light.Range, 0.00001);
        if (distanceSquared >= rangeSquared) {
            continue;
        }
        float distance = sqrt(distanceSquared);
        vec3 L = lightDirection / max(distance, 0.0001);
        float nDotL = max(dot(normal, L), 0.0);
        if (nDotL <= 0.0) {
            continue;
        }
        float attenuationDenom = light.Constant + light.Linear * distance + light.Quadratic * distanceSquared;
